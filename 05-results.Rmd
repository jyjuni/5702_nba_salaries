...
```{r, fig.cap=paste("      Figure 5: Distribution of categorical variables. ")}
library(mltools)
...
By looking closer into the distibution of salary in each race(Figure 6), it is notived that the distributions share similar range and shape, which means that the NBA league and teams in the league have no preference of race, and race is not an influential factor in deciding players' salary. 

```{r, fig.cap=paste("      Figure 6: Distribution of salary in each race. ")}
...
```{r, fig.cap=paste("      Figure 7: Alluvial graph of posistion, age interval, and salary interval. ")}
# alluvial graph
library(ggalluvial)
...
Then we explore the correlation between those numeric factors, which is shown below(Figure 9). 

```{r, fig.cap=paste("      Figure 9: Pairwise correlation matrix between numerical variables. ")}
...
```{r, warning=FALSE, fig.cap=paste("      Figure 10: Scatterplot between salary and its most correlated numerical variables individualy with linear regression abline.  ")}
...
```{r, warning=FALSE, fig.cap=paste("      Figure 11: Scatterplot between salary and VORP with linear regression abline and colored by groups from 4 categorical variables.  ")}
...
```
```{r, warning=FALSE, fig.cap=paste("      Figure 12: Geographic plot of salary median and maximum in each state in the US. ")}
library(statebins)
